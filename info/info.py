...
`3)` b!cookie | Give a user a cookie. (Requested by user.)""")
    ...
    async def stats(self, ctx):
        ...
        data.add_field(name="Voice Stats:", value="Connected to {} voice channels, with a total of {} users.".format(len([e.name for e in set(bot.get_all_channels()) if e.server.me.voice_channel == e]), len([e.voice_members for e in self.bot.get_all_channels() if e.server.me.voice_channel == e])), inline=False)
